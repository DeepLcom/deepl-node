...
describe('translate using glossaries', () => {
    ...
    it('should get glossaries', async () => {
        ...
        await expect(translator.getGlossary(invalidGlossaryId)).rejects.toThrowError(deepl.DeepLError);
        await expect(translator.getGlossary(nonExistentGlossaryId)).rejects.toThrowError(deepl.GlossaryNotFoundError);
    });
    ...
    it('should get glossary entries', async () => {
        ...
        try {
            expect((await translator.getGlossaryEntries(createdGlossary)).entries()).toStrictEqual(entries.entries());
            expect(await translator.getGlossaryEntries(createdGlossary.glossaryId)).toStrictEqual(entries);
        } finally {
            await cleanupGlossary();
        }

        await expect(translator.getGlossaryEntries(invalidGlossaryId)).rejects.toThrowError(deepl.DeepLError);
        await expect(translator.getGlossaryEntries(nonExistentGlossaryId)).rejects.toThrowError(deepl.GlossaryNotFoundError);
    });
    ...
    it('should delete glossaries', async () => {
        ...
        try {
            ...
            await expect(translator.getGlossary(createdGlossary.glossaryId)).rejects.toThrowError(deepl.GlossaryNotFoundError);
        } finally {
            await cleanupGlossary();
        }

        await expect(translator.deleteGlossary(invalidGlossaryId)).rejects.toThrowError(deepl.DeepLError);
        await expect(translator.deleteGlossary(nonExistentGlossaryId)).rejects.toThrowError(deepl.GlossaryNotFoundError);
    });
